...
class Critic(object):
    ...
    def __call__(self, x, x_):
        return tf.norm(self.h(x, reuse=True) - self.h(x_, reuse=True), axis=1) - tf.norm(self.h(x, reuse=True), axis=1)
...
class DisConv(object):
    ...
    def __call__(self, x, reuse=False):
        ...
        with tf.variable_scope(self.name) as scope:
            if reuse:
                scope.reuse_variables()
            d = convolution(x, [4, 4, 1, 64], strides = [1, 2, 2, 1], activation = leaky_relu, scope = 'conv1')
            d = flatten(d)
            d = fc_layer(d, 128, scope = "fc1")
            '''
            deprecated in cramer gan
