...
def optimizer(loss, var_list, initial_lr = 1e-4):
    ...
    '''
    return tf.train.AdamOptimizer(initial_lr).minimize(loss, var_list = var_list, colocate_gradients_with_ops=True)
...
def fc_layer(input_, output_size, activation = None, batch_norm = False, istrain = False, scope = None):
    ...
    with tf.variable_scope(scope or "fc"):
        ...
        if batch_norm:
            ...
            b = tf.get_variable(name="b", shape = [output_size], initializer=tf.constant_initializer(0.001))
            if activation is None:
